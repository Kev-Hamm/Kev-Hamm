...
- **Multi-agent orchestration** — task routing, dependency graphs, proof-gated handoffs, event-driven workers
- **Local-first model infra** — Ollama on consumer GPUs, LoRA/QLoRA fine-tuning, GGUF conversion, self-hosted inference across a node fleet
- **Production deploys** — immutable release dirs, systemd drop-ins, deploy locking, rollback by commit SHA
- **Unity + generative pipelines** — an open-world game port and an AI-assisted comic studio
...
Most of the interesting work lives in private repos — happy to walk through any of it.
